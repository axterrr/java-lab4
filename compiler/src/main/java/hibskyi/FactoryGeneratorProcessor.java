...
public class FactoryGeneratorProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> set, RoundEnvironment roundEnvironment) {
        ...
        for (Element annotatedElement : roundEnvironment.getElementsAnnotatedWith(FactoryGenerator.class)) {
            if (annotatedElement.getKind() != ElementKind.INTERFACE) {
                error("Only interface can be annotated with TransportFactory", annotatedElement);
                return true;
            }
        ...
        for (Element annotatedElement : roundEnvironment.getElementsAnnotatedWith(FactoryElement.class)) {
            if (annotatedElement.getKind() != ElementKind.CLASS) {
                error("Only class can be annotated with AutoElement", annotatedElement);
                return true;
            }
            FactoryElement factoryElement = annotatedElement.getAnnotation(FactoryElement.class);
            TypeElement typeElement = (TypeElement) annotatedElement;
            ClassName className = ClassName.get(typeElement);
            ...
            for (TypeMirror typeMirror : list) {
                ...
                if (result.containsKey(typeName)) {
                    result.get(typeName).add(new ElementInfo(factoryElement.value(), className));
                    break;
                }
            }
        }
    ...
    public Set<String> getSupportedAnnotationTypes() {
        ...
        annotations.add(FactoryGenerator.class.getCanonicalName());
        annotations.add(FactoryElement.class.getCanonicalName());
        return annotations;
    }
